...
class Users(db.Model):
    ...
    # association variable for many to many
    pinfo = db.relationship('cards', secondary='users_cards', backref='user')
    uaddress = db.relationship('address', secondary='users_address', backref='user')
...
class cards(db.Model):
    ...
    expr = db.Column(db.String(100))
    cvv = db.Column(db.Integer)
    #assignment says to manage credit
...
#association for many to many with users and cards
user_card = db.Table('users_cards',
db.Column('id',db.Integer,db.ForeignKey('users.id')),
db.Column('cid',db.Integer,db.ForeignKey('cards.cid'))
)
#association table for users and addresses
user_address = db.Table('users_address',
db.Column('id',db.Integer,db.ForeignKey('users.id')),
db.Column('aid',db.Integer,db.ForeignKey('address.aid'))
)
...
def index():
    ...
    # x = uuid.uuid4().int
    # n = Users(id=x, email='m@gmail.com', password='1234')
    # db.session.add(n)
    # db.session.commit()
    return "Username is m@gmail.com, password is 1234"
...
def addlocation():
    if not session:
        ...
        if request.method == "POST":
            addlocentry(request.form['street'],request.form['state'],request.form['city'],request.form['zip'])
        return render_template("SeansTempletes/addlocation.html")
def addlocentry(u_street,u_state,u_city,u_zip):
    if address.query.filter_by(street=u_street).count() > 0:
        return False
    else:
        uid = session["id"]
        u_a = Users.query.filter_by(id=uid).first()
        a_id = uuid.uuid4().int
        a = address(aid=a_id,street=u_street,state=u_state,city=u_city,zipcode=u_zip)
        db.session.add(a)
        db.session.commit()
        u_a.uaddress.append(a)
        db.session.commit()
        return True

#working add a ceiling of 2-3 cards
...
def addcard():
    if not session:
        ...
        if request.method == "POST":
            if addCardEntry(request.form['cnumber'],request.form['expr'],request.form['cvv'],request.form['credit']):
                uid = session["id"]
                u_c = Users.query.filter_by(id=uid).first()
                print(u_c.pinfo)
                return "<p>" + "nice" + "</p>"
            else:
                return "<p> xd </p>"
        return render_template("SeansTempletes/addcard.html")
def addCardEntry(u_number,u_expr,u_cvv,u_credit): #helper method for addcard
    #check if exists
    if cards.query.filter_by(cnumber=u_number).count() > 0:
        return False
    else:
        uid = session["id"]
        u_c = Users.query.filter_by(id=uid).first()
        s_cid = uuid.uuid4().int
        c = cards(cid=s_cid,expr=u_expr,cvv=u_cvv,credit=u_credit)
        db.session.add(c)
        db.session.commit()
        u_c.pinfo.append(c)
        db.session.commit()
...
def signup():
    if request.method == "POST":
        ...
        if Users.query.filter_by(email=u_email).count() > 0:
            return "<div>" + "shit" + "</div>"
        else:
            x = uuid.uuid4().int
            usr = Users(id=x,email=u_email,password=u_password)
            db.session.add(usr)
            db.session.commit()
            return redirect(url_for("login"))
    return render_template("SeansTempletes/signup.html")
...
def login():
    if request.method == "POST":
        u_email = request.form['email']
        u_password = request.form['password']
        info = Users.query.filter_by(email=u_email).first()
        if info:
            session["id"] = info.id
            session["email"] = info.email
            if info.email == u_email and info.password == u_password:
                return "<div>" + session["email"] +  "</div>"
        else:
            return "Please reenter credentials!"
    return render_template("SeansTempletes/login.html")
...
#puts users into pretty usertable
